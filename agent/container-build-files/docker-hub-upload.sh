...
count=`docker images | grep siteagent-main | grep latest | awk '{print $3}' | wc -l`
if [ "$count" -ne "1" ]; then
  echo "Count of docker images != 1. Which docker image you want to tag?"
  echo "Here is full list of docker images locally:"
  docker images | grep -i 'siteagent-main\|REPOSITORY'
  echo "Please enter IMAGE ID:"
  read dockerimageid
else
  dockerimageid=`docker images | grep siteagent-main | grep latest | awk '{print $3}'`
fi
...
today=`date +%Y%m%d`
if [ $ARCH = "x86_64" ]; then
  docker tag $dockerimageid sdnsense/site-agent-sense:$TAG-$today
  docker push sdnsense/site-agent-sense:$TAG-$today
  docker tag $dockerimageid sdnsense/site-agent-sense:$TAG
  docker push sdnsense/site-agent-sense:$TAG
elif [ $ARCH = "ppc64le" ]; then
  docker tag $dockerimageid sdnsense/site-agent-sense:$TAG-$ARCH-$today
  docker push sdnsense/site-agent-sense:$TAG-$ARCH-$today
  docker tag $dockerimageid sdnsense/site-agent-sense:$TAG-$ARCH
  docker push sdnsense/site-agent-sense:$TAG-$ARCH
fi
